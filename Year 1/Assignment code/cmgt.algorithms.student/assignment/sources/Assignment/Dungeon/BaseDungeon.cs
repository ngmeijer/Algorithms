...
namespace Dungeon
{
    public abstract class BaseDungeon : Canvas
    {
        ...
        private Pen wallPen = new Pen(Color.FromArgb(255, Color.Red));
        private Pen doorPen = Pens.Blue;

        /**
	 * Create empty dungeon instance of the specified size.
	 * It's empty because it doesn't contain any rooms yet.
	 */
        public BaseDungeon(Size pSize) : base(pSize.Width, pSize.Height)
        {
            size = pSize;
	 ...
	 */
        protected virtual void drawRoom(RoomContainer pRoomContainer, Pen pWallColor, Brush pFillColor = null)
        {
            ...
            if (pFillColor != null)
                graphics.FillRectangle(pFillColor, pRoomContainer.OriginalSize.Left, pRoomContainer.OriginalSize.Top,
                    pRoomContainer.OriginalSize.Width - 0.5f, pRoomContainer.OriginalSize.Height - 0.5f);
            graphics.DrawRectangle(pWallColor, pRoomContainer.OriginalSize.Left, pRoomContainer.OriginalSize.Top,
                pRoomContainer.OriginalSize.Width - 0.5f, pRoomContainer.OriginalSize.Height - 0.5f);
        }
